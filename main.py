...
import numpy as np
from src.utils.metric import get_ner_fmeasure
from src.model.seqlabel import SeqLabel
# from model.sentclassifier import SentClassifier
from src.utils.data import Data
from src.config import Config
# from eval import get_macro_avg
from src.eval_no_duplicate import get_macro_avg
...
def data_initialization(data):
    # data.initial_feature_alphabets() # no additional features added
...
def evaluate(data, model, name, nbest=None):
    ...
    for batch_id in range(total_batch):
        ...
        batch_word, batch_wordlen, batch_wordrecover, list_sent_words_tensor, batch_label, mask  = batchify_sequence_labeling_with_label(instance, False, False)
        tag_seq = model(batch_word, batch_wordlen, list_sent_words_tensor, mask)

        pred_label, gold_label = recover_label(tag_seq, batch_label, mask, data.label_alphabet, batch_wordrecover, False)# data.sentence_classification)
    ...
    if name == "raw":
        return speed, None, None, None, pred_results
    p, r, f = get_macro_avg(sequences, pred_results, doc_ids, dev_data_path=data.dev_input_dir)
...
def train(data):
    ...
    for idx in range(data.HP_iteration):
        ...
        for batch_id in range(total_batch):
            ...
            instance_count += 1
            # print(batch_word, batch_wordlen, list_sent_words_tensor, batch_label, mask)
            # print()
            # print(start, end)
            # print()
...
if __name__ == '__main__':
    ...
    if status == 'train':
        ...
        data.load(data.dset_dir)
        data.read_config(args.config)
        data.raw_dir = 'data_seq_tag_pairs/test'
        print(data.raw_dir)
