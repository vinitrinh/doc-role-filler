...
class Config:
    ...
    #     """
    #     This is when input data contains other features
    #     Author does not use this in published model
    #     """
    #     # if self.sentence_classification:
    ...
    def build_alphabet(self, input_file):
        ...
        for line in in_lines: 
            for word, label, idx in line['seq']:
            # if len(line) > 2:
                ## if sentence classification data format, splited by \t
                # if self.sentence_classification:
                ...
                # pairs = line.strip().split()   # ['we', 'O']
                # if len(pairs) < 2:
                #     continue
                # word = pairs[0]  # 'we'

                if self.number_normalized:
                    word = normalize_word(word) # JT: suspicious
                # label = pairs[-1]   # 'O'
                self.label_alphabet.add(label)
                self.word_alphabet.add(word)
    ...
    def generate_instance(self, name):
        self.fix_alphabet()
        if name == "train":
            self.train_texts, self.train_Ids = read_instance(self.train_dir, self.word_alphabet, self.char_alphabet, self.feature_alphabets, self.label_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH, self.split_token)
        elif name == "dev":
            self.dev_texts, self.dev_Ids = read_instance(self.dev_dir, self.word_alphabet, self.char_alphabet, self.feature_alphabets, self.label_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH, self.split_token)
        elif name == "test":
            self.test_texts, self.test_Ids = read_instance(self.test_dir, self.word_alphabet, self.char_alphabet, self.feature_alphabets, self.label_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH, self.split_token)
        elif name == "raw":
            self.raw_texts, self.raw_Ids = read_instance(self.raw_dir, self.word_alphabet, self.char_alphabet, self.feature_alphabets, self.label_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH, self.split_token)
        else:
    ...
    def read_config(self,config_file):
        ...
        the_item = 'dev_input_dir'
        if the_item in config:
            self.dev_input_dir = config[the_item]
        the_item = 'test_input_dir'
        if the_item in config:
            self.test_input_dir = config[the_item]
        the_item = 'raw_dir'
        if the_item in config:
