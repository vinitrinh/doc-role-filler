...
tokenizer = BertTokenizer.from_pretrained('bert-base-uncased', do_lower_case=True, use_fast=False) # Load bert pre-trained model tokenizer (vocabulary)
...
tags_to_extract =['adversary', 'capability', 'infrastructure', 'victim']
tag2category = {'adversary': "adversary", 'capability':"capability", 'infrastructure':"infrastructure", 'victim':"victim"}
...
def get_eval_results(doc_pred_spans, doc_gold_events, to_print=False):
    ...
    for doc_id in doc_pred_spans:
        ...
        for tag in tags_to_extract:
            doc_pred_spans_head_noun[doc_id][tag.lower()] = list()
            for idx, span in enumerate(doc_pred_spans[doc_id][tag.lower()]):
                span_tokenized = tokenizer.tokenize(span) # to normalize (remove other diff between pred and gold)
                doc_pred_spans[doc_id][tag.lower()][idx] = span_tokenized
                ...
                doc_pred_spans_head_noun[doc_id][tag.lower()].append({"span": span_tokenized, "hn": head_noun})

            doc_pred_spans[doc_id][tag.lower()] = remove_duplicate(doc_pred_spans[doc_id][tag.lower()])
            doc_pred_spans_head_noun[doc_id][tag.lower()] = remove_duplicate_head_noun(doc_pred_spans_head_noun[doc_id][tag.lower()])
    ...
    for doc_id in doc_gold_events:
        ...
        for tag in tags_to_extract:
            doc_gold_events_head_noun[doc_id][tag.lower()] = list()
            # print(doc_gold_events[doc_id]["roles"])
            for idx, event in enumerate(doc_gold_events[doc_id]["roles"][tag.lower()]):
                event_tokenized = []
                event_head_noun = []
                ...
                doc_gold_events[doc_id]["roles"][tag.lower()][idx] = event_tokenized
                doc_gold_events_head_noun[doc_id][tag.lower()].append(event_head_noun)
    ...
    for tag in tags_to_extract:
        ...
        for doc_id in doc_gold_events:
            gold_events = doc_gold_events[doc_id]["roles"][tag.lower()]
            if doc_id not in doc_pred_spans: continue
            pred_spans = doc_pred_spans[doc_id][tag.lower()]
        ...
        if to_print:
            print("%s\n%.4f %.4f %.4f"%(tag2category[tag.lower()], prec, recall, f_measure))
    ...
    for tag in tags_to_extract:
        gold_event_num, right_event_num, pred_span_num, right_span_num = 0, 0, 0, 0
        for doc_id in doc_gold_events:
            gold_events = doc_gold_events[doc_id][tag.lower()]
            if doc_id not in doc_pred_spans: continue
            pred_spans = doc_pred_spans[doc_id][tag.lower()]
...
def get_macro_avg(sequences, pred_results, doc_ids, dev_data_path="data/input/cybersecurity-reports/dev_full.json"):
    # read pred spans
    doc_pred_spans = dict()
    for seq, pred_seq, doc_id in zip(sequences, pred_results, doc_ids):
        ...
        if doc_id not in doc_pred_spans:
            ...
            for tag in tags_to_extract:
                doc_pred_spans[doc_id][tag.lower()] = list()
    ...
    # read gold events
    with open(dev_data_path) as f_gold:
